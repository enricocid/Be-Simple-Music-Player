...
class MainActivity : AppCompatActivity(), UIControlInterface {
    ...
    // Colors
    private val mResolvedAccentColor by lazy { R.color.blue.decodeColor(this) }

    private val mResolvedIconsColor by lazy { R.color.widgetsColor.decodeColor(this) }

    private val mRoundedButtonsBackground by lazy {
        ...
        MaterialShapeDrawable(shapeAppearanceModel).apply {
            fillColor = ColorStateList.valueOf(mResolvedAccentColor)
        }
    }
    ...
    private fun handleFragmentsButton(index: Int) {
        mFragmentButtons.iterator().withIndex().forEach {
            ...
            val color = if (isButtonSelected) {
                Pair(R.color.windowBackground.decodeColor(this), mRoundedButtonsBackground)
            } else {
                Pair(mResolvedIconsColor, null)
            }
    ...
    private fun initMediaButtons() {
        mPlayerControlsPanelBinding.apply {
            ...
            songProgress.setLabelFormatter {
                return@setLabelFormatter it.toLong().toFormattedDuration(isAlbum = false, isSeekBar = true)
            }
    ...
    private fun updateRepeatStatus(onPlaybackCompletion: Boolean) {
        ...
        mPlayerControlsPanelBinding.apply {
            ...
            repeat.setImageResource(ThemeHelper.getRepeatIcon(
                    mMediaPlayerHolder
            ))
